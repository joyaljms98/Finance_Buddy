...
import { Plus, Search, BookOpen, Edit, Trash2, ExternalLink, PanelLeftOpen, PanelLeftClose, Folder, FolderPlus, Grid, List as ListIcon } from 'lucide-react';
...
export default function AdminArticlesPage() {
    ...
    // Mock Folders (Kept for frontend UI structure as requested)
        ...
        try {
            if (data.id) {
                ...
                const res = await api.put(`/articles/${data.id}`, data, {
                    headers: {
                        Authorization: `Bearer ${localStorage.getItem('finance_buddy_token')}`
                    }
                });
            ...
            alert("Failed to save article. Make sure you are logged in as Admin or Editor.");
        }
    };
                        ...
                        <div>
                            <h1 className="text-xl font-bold text-gray-900 flex items-center gap-3">
                                ...
                                <button 
                                    ...
                                    className="p-1 text-gray-400 hover:text-blue-600 bg-gray-100 hover:bg-blue-50 rounded-md transition-colors font-normal text-xs flex items-center gap-1 hidden md:flex"
                                    title="Toggle Folders Sidebar"
                                >
                                    <Folder size={14} /> {isLocalSidebarOpen ? 'Hide Folders' : 'Show Folders'}
                                </button>
                            </h1>
                                                    ...
                                                    <td className="p-4">
                                                        <div className="flex items-center gap-3">
                                                            ...
                                                            <div>
                                                                ...
                                                                <p className="text-xs text-gray-500">by {((article.authors || []).join(', ') || 'Unknown Author')}</p>
                                                            </div>
                                                        </div>
                                                    </td>
                                                    <td className="p-4">
                                        ...
                                        <div key={article.id} className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden hover:shadow-md transition group flex flex-col">
                                            ...
                                            <div className="p-5 flex flex-col flex-1">
                                                ...
                                                <div className="flex justify-between items-center mt-auto border-t border-gray-50 pt-4">
                                                    <span className="text-xs text-gray-400 font-medium">{((article.authors || []).join(', ') || 'Unknown Author')}</span>
